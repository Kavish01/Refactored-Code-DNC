angular
    ...
    .directive('bannerModule', ['$compile', "$sce","$interval", "content", "configService",  function(compile, sce, interval, content, configService) {
        return {
            restrict: 'A',
            template: '<div id="bannerHeader" ng-show="headerTxt.length>0 && orientation==true" class="moduleHeaderStyle">{{headerTxt}}</div><div ><img ng-src="'+currentAssetPath+'/Banner/{{videoSource}}" class="bannerContiner"/></div>',
            scope: {
              load:"=",
			  orientation:"="
            },
...
scope.headerTxt = labelText._Label
                  ...
                  onTimeout = function()
					...
					if(videoData.length != undefined)
					{
						 videoSwitchInterval = interval(onTimeout,configService.get("bannerModuleInterval"));
					}
